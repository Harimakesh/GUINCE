...
typedef struct {
    ...
    int pceNum[13];       // Number of certain piece in the board
    int bigPce[2];       // Anything that isn't a pawn
    int majPce[2];      // Rooks and queens
    int minPce[2];     // bishops and nights
    int material[2];     
...
First 7 bits -> from
next 7 bits -> to
next 4 bits -> captured
next bit  -> Empassant
next bit  -> pawn start 
next 4 bits -> promoted piece
next 1 bit -> castle
